import type { OpenSocialNetworkFollow, OpenSocialNetworkFollowList } from './types';
...
export function followsFromFollowList(
  owner: string,
  value: unknown,
): string[] {
  if (!isOpenSocialNetworkFollowList(value) || value.owner !== owner) {
    return [];
  }

  return value.follows.map((follow) => follow.profile);
}
